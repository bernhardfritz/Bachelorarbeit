...
Of course, there are some areas that could potentially be improved by more sophisticated implementations that take full advantage of the hardware available.  Heightmap transformations are currently CPU-based, using a data structure similar to two-dimensional arrays. GPU-based data structures like textures would be considerably faster for this type of application. Unfortunately the changes required are non-trivial and a complete overhaul of the erosion algorithms as well as the tectonic plate simulation would be necessary.

The SPH-based hydraulic erosion simulation is CPU-bound as well. Due to this only a very limited amount of about 3000 simultaneously moving particles can be used. GPUs would be much more capable to handle the concurrent computations required for simulating fluid particle motion. Using a larger quantity of particles would also imply the usage of smaller particles. This could potentially result in even more realistic hydraulic erosion landscape transformations.

The most noticeable types of terrain erosion are thermal and hydraulic erosion. Both of which were implemented in the course of this bachelor thesis. Nevertheless, also other types of erosion like wind and glacial erosion have a significant impact on landscape and could be implemented in future.

Although the presented tectonic plate simulation is perfectly capable to model plate movement and collisions, it effectively accounts only for a quite limited amount of phenomena involved in plate tectonics. To give an example, subducting and diverging plates are not properly supported. Currently plates are only split during the initialization process of the simulation. In future implementations, plates could be split during the simulation process if force thresholds are exceeded. Other than that, volcanic activity and earthquakes could also be subject of further research, in terms of how these phenomena could be simulated in a virtual environment. The current physics engine in use is highly simplified and only accounts for 2 dimensions. A more sophisticated implementation could make use of all 3 dimensions and in this way be able to simulate subducting plates, namely plates overlapping each other. Obviously, there always is a trade-off between performance and level of detail to be considered when deciding to use more sophisticated models instead of simple yet effective alternatives.

To improve the visual appearance of the scenery it would be possible to procedurally generate textures instead of using the same seamless textures over and over. Although barely noticeable, under close inspection repeating texture details can be spotted.
Shadows are currently not supported. Frequently changing meshes like heightmaps are not suited for shadow mapping since it would be too expensive to generate a new shadow map each time the heightmap gets altered. A possible solution to this problem would be generating shadow maps only occasionally, e.g. when taking screeshots.
Water waves are currently implemented by combining sine waves with OpenSimplexNoise to continuously alter a second semi-transparent heightmap consisting of 256x256 tiles with an animated diffuse water texture. To improve the performance of water waves it would make sense to remove the additional water heightmap and use an animated specular and normal texture to mimic water waves on a single tile instead.
