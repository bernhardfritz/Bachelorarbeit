...
float MarchingCubes::fSample1(float fX, float fY, float fZ, vector<VEC3D*>& positions, float radius)
{
    double fResult = 0.0;
    double fDx, fDy, fDz;
...
void MarchingCubes::vGetNormal(vec3 &rfNormal, float fX, float fY, float fZ, vector<VEC3D*>& positions, float radius)
{
...
void MarchingCubes::vMarchCube1(float fX, float fY, float fZ, float fScale, vector<VEC3D*>& positions, float radius, MetaMesh& metamesh)
{
    ...
    for(iTriangle = 0; iTriangle < 5; iTriangle++)
    {
        ...
        for(iCorner = 0; iCorner < 3; iCorner++)
        {
            ...
            metamesh.addNormal(vec3(asEdgeNorm[iVertex].x, asEdgeNorm[iVertex].y, asEdgeNorm[iVertex].z));
            metamesh.addVertex(vec3(asEdgeVertex[iVertex].x, asEdgeVertex[iVertex].y, asEdgeVertex[iVertex].z));
        }
    }
}

//vMarchingCubes iterates over the entire dataset, calling vMarchCube on each cube
void MarchingCubes::vMarchingCubes(vector<VEC3D*>& positions, float radius, MetaMesh& metamesh)
{
    metamesh.clear();
    int iX, iY, iZ;
    ...
    {
        vMarchCube1(iX*fStepSize, iY*fStepSize, iZ*fStepSize, fStepSize, positions, radius, metamesh);
    }
    metamesh.update();
}